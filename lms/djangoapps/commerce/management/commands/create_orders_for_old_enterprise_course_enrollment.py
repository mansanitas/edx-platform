...
class Command(BaseCommand):
    ...
    def _create_manual_enrollment_orders(self, enrollments):
        ...
        return len(successful_creations), len(failed_creations)
    ...
    def _sync_with_ecommerce(self, enrollments_batch):
        ...
        for enrollment in enrollments_batch:
            try:
                ...
                self.stdout.write(u'\t\tskipping enrollment {} due to invalid data. {}'.format(enrollment, ex))
                invalid += 1
                continue
            except CourseEnrollment.DoesNotExist:
                self.stdout.write(u'\t\tskipping enrollment {} because CourseEnrollment not found'.format(enrollment))
                invalid += 1
                continue
        ...
        if not enrollments_payload:
            return 0, 0, invalid, non_paid

        self.stdout.write(u'\t\tSyncing started...')
        success, failed = self._create_manual_enrollment_orders(enrollments_payload)
        self.stdout.write(
            u'\t\tSuccess: {} , Failed: {}, Invalid:{} , Non-Paid: {}'.format(
                success, failed, invalid, non_paid
            )
        )
        return success, failed, invalid, non_paid
    ...
    def _sync(self, enrollments_queryset, enrollments_count, enrollments_batch_size):
        ...
        while offset < enrollments_count:
            ...
            while len(enrollments_queue) >= enrollments_batch_size \
                ...
                success, failed, invalid, non_paid = self._sync_with_ecommerce(enrollments_batch)
        ...
        self.stdout.write(
            u'[Final Summary] Enrollments Success: {} , Failed: {}, Invalid: {} , Non-Paid: {}'.format(
                successfully_synced_enrollments, failed_to_synced_enrollments, invalid_enrollments, non_paid_enrollments
            )
        )
