...
class TestEnterpriseCourseEnrollmentCreateOldOrder(TestCase):
    ...
    def test_command(self, mock_create_manual_enrollment_orders):
        ...
        """
        mock_create_manual_enrollment_orders.return_value = (0, 0)  # not correct return value, just fixing unpacking
        out = StringIO()
        ...
        self.assertTrue(
            re.search(r'\[Final Summary\] Enrollments Success: \d+ , Failed: 0, Invalid: 1 , Non-Paid: 1', output)
        )
    ...
    def test_command_start_and_end_index(self, mock_create_manual_enrollment_orders):
        """
            Test command with batch size
        """
        mock_create_manual_enrollment_orders.return_value = (0, 0)  # not correct return value, just fixes unpacking
        out = StringIO()
        call_command(
        ...
        self.assertEqual(mock_create_manual_enrollment_orders.call_count, 2)  # batch of 2 (15, 5)
